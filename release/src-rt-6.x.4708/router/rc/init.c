...
#ifdef TCONFIG_AC5300
	else if (model == MODEL_EA9400 || model == MODEL_EA9500) {
    etxmac = nvram_safe_get("et2macaddr"); /* base LAN MAC on EA9x00 (GMAC3 switch) */
    if (!nvram_match("lan_hwaddr", (char *)etxmac) && (strlen(etxmac) >= 17)) {
        nvram_set("lan_hwaddr", etxmac);
        nvram_set("et0macaddr", etxmac);
    }
}
...
static void check_bootnv(void)
{
	...
	case MODEL_EA9500:
    	...
    	nvram_unset("et1mdcport");
    	nvram_unset("et1phyaddr");
 		dirty |= check_nv("rgmii_port", "5");
    	dirty |= check_nv("vlan1hwname", "et2");
...
static int init_nvram(void)
{
	...
	case MODEL_EA9500:
    mfr = "Linksys";
    name = (model == MODEL_EA9400) ? "EA9400" : "EA9500";
    features = SUP_SES | SUP_80211N | SUP_1000ET | SUP_80211AC;
#ifdef TCONFIG_USB
    nvram_set("usb_uhci", "-1");
#endif
    if (!nvram_match("t_fix1", (char *)name)) {
        /* LAN/WAN interface setup - switch is et2 (GMAC3), confirmed from nvram */
        ...
        nvram_set("wl_ifname", "eth1");
        nvram_set("wl0_ifname", "eth1");   /* 2.4GHz */
        nvram_set("wl1_ifname", "eth2");   /* 5GHz low */
        nvram_set("wl2_ifname", "eth3");   /* 5GHz high */
        nvram_set("wl0_vifnames", "wl0.1 wl0.2 wl0.3");
        nvram_set("wl1_vifnames", "wl1.1 wl1.2 wl1.3");
        nvram_set("wl2_vifnames", "wl2.1 wl2.2 wl2.3");

        /* MAC addresses - hw_mac_addr holds the real base MAC (...:35)
         * et0macaddr is zeroed in hardware, derive from hw_mac_addr */
        strlcpy(s, nvram_safe_get("hw_mac_addr"), sizeof(s));
        nvram_set("et0macaddr", s);         /* set LAN base MAC */
        nvram_set("lan_hwaddr", s);
        /* radio MACs come from nvram as-is (1:macaddr, 2:macaddr, 3:macaddr) */
        nvram_set("wl0_hwaddr", nvram_safe_get("2:macaddr")); /* 2.4GHz = eth1 */
        nvram_set("wl1_hwaddr", nvram_safe_get("1:macaddr")); /* 5GHz-1 = eth2 */
        nvram_set("wl2_hwaddr", nvram_safe_get("3:macaddr")); /* 5GHz-2 = eth3 */

        /* GMAC3 forwarding - preserve stock values */
        ...
        /* wifi channels - tri-band defaults */
        nvram_set("wl0_bw_cap", "3");
        nvram_set("wl0_chanspec", "6u");
        nvram_set("wl0_channel", "6");
        nvram_set("wl0_nbw", "40");
        ...
        /* devpaths - confirmed from nvram dump */
        nvram_set("devpath1", "pcie/1/4/");
	...
	for (;;) {
		...
		case SIGUSR2: /* START */
			...
			if (restore_defaults_fb /*|| nvram_match("wireless_restart_req", "1")*/) {
				...
				if (foreach_wif(1, NULL, disabled_wl_vif)) {
